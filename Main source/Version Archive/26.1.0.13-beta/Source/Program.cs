...
namespace PulseMute
{
    ...
    internal sealed partial class MainForm : Form
    {
        ...
        private bool showHotkeyStrip = true;
        ...
        private void ShowSettingsV15Legacy()
        {
            using (Form dialog = new Form())
            {
                ...
                Label hotkeyStripLabel = CreateSettingsLabel("Show hotkey strip", 20, 840);
                Label hotkeyStripState = CreateSettingsStateLabel(20, 862);
                ToggleSwitch hotkeyStripToggle = CreateToggleSwitch(322, 846, showHotkeyStrip);
                ...
                Action applyDialogTheme = delegate
                {
                    ...
                    hotkeyStripState.Text = hotkeyStripToggle.Checked ? "Visible below creator line" : "Hidden";
                ...
                {
                    showHotkeyStrip = hotkeyStripToggle.Checked;
                    ApplyResponsiveLayout();
                    SaveSettingsFile();
                    applyDialogTheme();
                };
        ...
        private static string EditionVersionText()
        {
            ...
            return "PulseMute Beta 26.1.0.13-beta";
        }
        ...
        private void ApplyResponsiveLayout()
        {
            ...
            shortcutValueLabel.Visible = showHotkeyStrip;
            shortcutTwoValueLabel.Visible = showHotkeyStrip;
            ...
            int shortcutTop = bottomTop - gap - shortcutHeight;
            int creditHeight = Clamp((int)(20 * scale), 16, 28);
            int creditTop = (showHotkeyStrip ? shortcutTop : bottomTop) - gap - creditHeight;
            ...
            int circleBottom = creditTop - gap;
        ...
        private void LoadWindowSettings()
        {
            ...
            try
            {
                ...
                foreach (string line in lines)
                {
                    ...
                    else if (line.StartsWith("ShowHotkeyStrip=", StringComparison.OrdinalIgnoreCase))
                    {
                        bool.TryParse(line.Substring("ShowHotkeyStrip=".Length), out showHotkeyStrip);
                    }
        ...
        private void SaveSettingsFile()
        {
            ...
            File.WriteAllText(
                ...
                "SoundVolume=" + soundVolume + Environment.NewLine +
                "AnimationsEnabled=" + animationsEnabled + Environment.NewLine +
                "ShowHotkeyStrip=" + showHotkeyStrip + Environment.NewLine +
                "LegacySettingsEnabled=" + legacySettingsEnabled + Environment.NewLine +
        ...
        {
            return Path.Combine(appDataRoot, "PulseMute", "Instances", InstanceIdentityForExecutable(executablePath), "settings.txt");
        }

        internal static string AutoStartValueNameForExecutable(string executablePath)
        {
            return "PulseMuteAutoStart-" + InstanceIdentityForExecutable(executablePath);
        }
        ...
        private static void RemoveMatchingLegacyAutoStart()
        {
            try
            {
                using (RegistryKey key = Registry.CurrentUser.OpenSubKey(@"Software\Microsoft\Windows\CurrentVersion\Run", true))
                {
                    if (key == null)
                        return;
                    string legacyCommand = Convert.ToString(key.GetValue("PulseMuteBetaAutoStart"));
                    if (string.IsNullOrWhiteSpace(legacyCommand))
                        return;
                    string executable = Application.ExecutablePath.Trim();
                    string command = legacyCommand.Trim().Trim('"');
