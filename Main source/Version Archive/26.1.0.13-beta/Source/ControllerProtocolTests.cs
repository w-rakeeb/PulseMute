...
namespace PulseMute
{
    internal static class ControllerProtocolTests
    {
        ...
        private static void TestResponsiveHotkeyLayout()
        {
            ...
            System.Windows.Forms.Control mute = (System.Windows.Forms.Control)type.GetField("toggleButton", flags).GetValue(form);
            ...
            try
            {
                ...
                foreach (System.Drawing.Size size in sizes)
                {
                    ...
                    Assert(keyOne.Bottom < refresh.Top, "Hotkey strip overlapped the bottom controls at " + size + ".");
                }

                form.ClientSize = new System.Drawing.Size(284, 341);
                applyLayout.Invoke(form, null);
                int visibleMuteSize = mute.Height;
                showHotkeyStrip.SetValue(form, false);
                applyLayout.Invoke(form, null);
                Assert(!keyOne.Visible && !keyTwo.Visible, "Developer option did not hide both hotkey cards.");
                Assert(mute.Height >= visibleMuteSize, "Hiding the hotkey strip did not release space to the mute control.");
        ...
        private static void TestArchivedVersionDiscovery()
        {
            ...
            Assert(versions.Count >= 22, "Expected the Beta snapshots and legacy releases in the version picker.");
        ...
        private static void TestNativeSettingsDesign()
        {
            ...
            Assert(source.IndexOf("26.1.0.13-beta", StringComparison.Ordinal) >= 0, "Settings version label is stale.");
