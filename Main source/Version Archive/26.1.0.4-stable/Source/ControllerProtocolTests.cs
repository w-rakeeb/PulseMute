...
namespace PulseMute
{
    internal static class ControllerProtocolTests
    {
        ...
        private static void TestArchivedVersionDiscovery()
        {
            ...
            Assert(versions.Count >= 13, "Expected the Stable snapshot and legacy releases in the version picker.");

            bool foundStableSnapshot = false;
            ...
            foreach (ArchivedVersionInfo version in versions)
            {
                ...
                if (version.DisplayName == "Stable 26.1.0.3-stable")
                    foundStableSnapshot = version.Details.IndexOf("hotkey", StringComparison.OrdinalIgnoreCase) >= 0;
            }
            Assert(foundStableSnapshot, "Stable 26.1.0.3 backup or its version details were not discovered.");
