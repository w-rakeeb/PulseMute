...
namespace PulseMute
{
    internal static class ControllerProtocolTests
    {
        ...
        private static void TestInstanceIsolation()
        {
            ...
            string previousReleaseConfig = MainForm.ConfigPathForRelease(stablePath, appData, "Stable", "26.1.0.5-stable");
            string currentReleaseConfig = MainForm.ConfigPathForRelease(stablePath, appData, "Stable", "26.1.0.6-stable");
            ...
            Assert(MainForm.ReleaseSettingsIdentity().IndexOf("Stable-26_1_0_6-stable", StringComparison.OrdinalIgnoreCase) >= 0, "Current Stable release identity is invalid.");
        }
        ...
        private static void TestArchivedVersionDiscovery()
        {
            ...
            Assert(versions.Count >= 16, "Expected Stable snapshots and legacy releases in the version picker.");
        ...
        private static void TestNativeSettingsDesign()
        {
            ...
            Assert(source.IndexOf("26.1.0.6-stable", StringComparison.Ordinal) >= 0, "Settings version label is stale.");
            ...
            Assert(programSource.IndexOf("private int logoStyle = 2;", StringComparison.Ordinal) >= 0, "Rounded square is not the default app logo.");
            Assert(System.IO.File.ReadAllText(System.IO.Path.Combine(appDirectory, "Main source", "build.ps1")).IndexOf("PulseMute-logo-rounded.ico", StringComparison.Ordinal) >= 0,
                "Rounded square is not the executable icon.");
