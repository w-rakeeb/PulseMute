...
namespace PulseMute
{
    internal static class ControllerProtocolTests
    {
        ...
        private static void TestInstanceIsolation()
        {
            ...
            string previousReleaseConfig = MainForm.ConfigPathForRelease(stablePath, appData, "Stable", "26.1.0.6-stable");
            string currentReleaseConfig = MainForm.ConfigPathForRelease(stablePath, appData, "Stable", "26.1.0.7-stable");
            ...
            Assert(MainForm.ReleaseSettingsIdentity().IndexOf("Stable-26_1_0_7-stable", StringComparison.OrdinalIgnoreCase) >= 0, "Current Stable release identity is invalid.");
        }
        ...
        private static void TestArchivedVersionDiscovery()
        {
            ...
            Assert(versions.Count >= 17, "Expected Stable snapshots and legacy releases in the version picker.");
        ...
        private static void TestNativeSettingsDesign()
        {
            ...
            Assert(source.IndexOf("26.1.0.7-stable", StringComparison.Ordinal) >= 0, "Settings version label is stale.");
            ...
            Assert(System.IO.File.ReadAllText(System.IO.Path.Combine(appDirectory, "Main source", "build.ps1")).IndexOf("$icon = Join-Path $root \"PulseMute.ico\"", StringComparison.Ordinal) >= 0,
                "Supplied primary artwork is not the executable icon.");
            Assert(System.IO.File.Exists(System.IO.Path.Combine(appDirectory, "Main source", "PulseMute-primary.svg")), "Supplied primary SVG source is missing.");
        ...
        private static void TestProfessionalLogoResource()
        {
            ...
            using (System.Drawing.Bitmap logo = MainForm.CreateHighQualityLogoBitmap())
            {
                ...
                Assert(primaryRed.R == 251 && primaryRed.G == 69 && primaryRed.B == 61, "Primary logo is not the supplied #FB453D artwork.");
            }
