...
$testRoot = Join-Path $env:TEMP "PulseMute-Test-Controller-Tests"
...
if ($LASTEXITCODE -ne 0) {
    throw "Input test build failed with exit code $LASTEXITCODE"
}

& $target
if ($LASTEXITCODE -ne 0) {
    throw "Input tests failed with exit code $LASTEXITCODE"
}
