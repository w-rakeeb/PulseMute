...
namespace PulseMute
{
    ...
    internal sealed class MainForm : Form
    {
        ...
        private void HideToTray()
        {
            ...
            tray.ShowBalloonTip(900, "PulseMute PS", "Listening for " + CurrentShortcutText(1) + " or " + CurrentShortcutText(2) + ".", ToolTipIcon.Info);
        }
        ...
        private static bool IsAutoStartEnabled()
        {
            ...
            {
                return key != null && key.GetValue("PulseMutePS11AutoStart") != null;
            }
        }
        ...
        private static void SetAutoStartEnabled(bool enabled)
        {
            using (RegistryKey key = Registry.CurrentUser.CreateSubKey(@"Software\Microsoft\Windows\CurrentVersion\Run"))
            {
                ...
                if (enabled)
                    key.SetValue("PulseMutePS11AutoStart", "\"" + Application.ExecutablePath + "\"");
                else
                    key.DeleteValue("PulseMutePS11AutoStart", false);
            }
        }

        private static string ConfigPath()
        {
            return Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "PulseMute", "PS 1.1", "settings.txt");
        }
