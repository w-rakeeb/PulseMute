...
namespace PulseMute
{
    internal static class ControllerProtocolTests
    {
        ...
        private static void TestInstanceIsolation()
        {
            ...
            string previousReleaseConfig = MainForm.ConfigPathForRelease(betaPath, appData, "Beta", "26.1.0.18-beta");
            string currentReleaseConfig = MainForm.ConfigPathForRelease(betaPath, appData, "Beta", "26.1.0.19-beta");
            ...
            Assert(MainForm.ReleaseSettingsIdentity().IndexOf("Beta-26_1_0_19-beta", StringComparison.OrdinalIgnoreCase) >= 0, "Current Beta release identity is invalid.");
        }
        ...
        private static void TestArchivedVersionDiscovery()
        {
            ...
            Assert(versions.Count >= 28, "Expected the Beta snapshots and legacy releases in the version picker.");
        ...
        private static void TestNativeSettingsDesign()
        {
            ...
            Assert(source.IndexOf("26.1.0.19-beta", StringComparison.Ordinal) >= 0, "Settings version label is stale.");
            ...
            Assert(source.IndexOf("Record button", StringComparison.Ordinal) >= 0 && source.IndexOf("Power ring", StringComparison.Ordinal) >= 0, "Replacement mute visuals are missing.");
            Assert(source.IndexOf("Shield mic", StringComparison.Ordinal) < 0 && source.IndexOf("Signal hex", StringComparison.Ordinal) < 0, "Rejected mute visuals are still selectable.");
        ...
        private static void TestProfessionalLogoResource()
        {
            ...
            {
                Assert(logo.Width == 256 && logo.Height == 256, "High-quality About logo resource dimensions are invalid.");
            }
        }
