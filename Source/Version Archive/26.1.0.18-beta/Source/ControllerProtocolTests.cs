...
namespace PulseMute
{
    internal static class ControllerProtocolTests
    {
        ...
        private static void TestInstanceIsolation()
        {
            ...
            string previousReleaseConfig = MainForm.ConfigPathForRelease(betaPath, appData, "Beta", "26.1.0.17-beta");
            string currentReleaseConfig = MainForm.ConfigPathForRelease(betaPath, appData, "Beta", "26.1.0.18-beta");
            ...
            Assert(MainForm.ReleaseSettingsIdentity().IndexOf("Beta-26_1_0_18-beta", StringComparison.OrdinalIgnoreCase) >= 0, "Current Beta release identity is invalid.");
        }

        private static void TestCompactSettingsCardLayout()
        {
            System.Windows.Forms.Panel page = new System.Windows.Forms.Panel();
            page.Size = new System.Drawing.Size(436, 384);
            ...
            Assert(cards.Count == 1 && cards[0].Width == 392, "Compact Settings card did not use the real content width.");
            Assert(title.Width >= 260 && state.Width >= 260, "Settings text collapsed beside its toggle.");
        ...
        private static void TestSettingsPlacement()
        {
            ...
            System.Drawing.Size settingsSize = new System.Drawing.Size(600, 460);
        ...
        private static void TestArchivedVersionDiscovery()
        {
            ...
            Assert(versions.Count >= 27, "Expected the Beta snapshots and legacy releases in the version picker.");
        ...
        private static void TestNativeSettingsDesign()
        {
            ...
            Assert(source.IndexOf("ClientSize = new Size(600, 460)", StringComparison.Ordinal) >= 0, "Compact Settings size is missing.");
            Assert(source.IndexOf("MinimumSize = new Size(560, 420)", StringComparison.Ordinal) >= 0, "Compact Settings minimum size is missing.");
            Assert(source.IndexOf("sidebar.SetBounds(0, 36, 164, 424)", StringComparison.Ordinal) >= 0, "Minimal Settings sidebar dimensions are missing.");
            Assert(source.IndexOf("button.SetBounds(10, y, 144, 34)", StringComparison.Ordinal) >= 0, "Compact navigation dimensions are missing.");
            ...
            Assert(source.IndexOf("26.1.0.18-beta", StringComparison.Ordinal) >= 0, "Settings version label is stale.");
            ...
            Assert(source.IndexOf("page.Size = new Size(436, 384)", StringComparison.Ordinal) >= 0, "Settings pages still use the oversized design canvas.");
