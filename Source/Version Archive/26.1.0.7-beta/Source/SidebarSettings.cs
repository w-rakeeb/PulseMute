    ...
    internal sealed partial class MainForm
    {
        private void ShowSettingsV15()
        {
            using (Form dialog = new BufferedSettingsForm())
            {
                ...
                developerContent.SetBounds(28, 166, 510, 440);
                ...
                Label roundedControlsLabel = AddPageLabel(developerContent, primaryLabels, "Rounded controls", 0, 64);
                Label roundedControlsState = AddPageState(developerContent, stateLabels, 0, 86);
                ToggleSwitch roundedControlsToggle = AddPageToggle(developerContent, toggles, 464, 70, roundedControlsEnabled);

                Label logoStyleLabel = AddPageLabel(developerContent, primaryLabels, "App logo", 0, 128);
                ThemedComboBox logoStyleBox = CreatePageComboBox(0, 156, 510);
                logoStyleBox.Items.Add("Current logo");
                logoStyleBox.Items.Add("Professional preview");
                logoStyleBox.SelectedIndex = string.Equals(logoStyle, "Professional", StringComparison.OrdinalIgnoreCase) ? 1 : 0;
                developerContent.Controls.Add(logoStyleBox);
                comboBoxes.Add(logoStyleBox);

                Panel developerOptionsSeparator = CreateSeparator(0, 204, 510);
                ...
                Label olderVersionLabel = AddPageLabel(developerContent, primaryLabels, "Older version", 0, 226);
                ThemedComboBox olderVersionBox = CreatePageComboBox(0, 254, 382);
                ...
                Button openVersionButton = CreateSmallButton("Open", new Point(392, 254));
                ...
                Label versionsInfoLabel = AddPageLabel(developerContent, primaryLabels, "Versions info", 0, 306);
                Label versionInfoText = new Label();
                versionInfoText.AutoSize = false;
                versionInfoText.SetBounds(0, 334, 510, 74);
                ...
                aboutVersion.Text = "Version 26.1.0.7-beta";
                ...
                sidebarVersion.Text = "26.1.0.7-beta";
                ...
                applyDialogTheme = delegate
                {
                    ...
                    roundedControlsState.Text = roundedControlsToggle.Checked ? "Enabled" : "Disabled";
                    ...
                    developerPage.AutoScrollMinSize = developerModeToggle.Checked ? new Size(0, 640) : new Size(0, 410);
                    ...
                    foreach (Button button in actionButtons)
                        SetControlRounded(button, roundedControlsEnabled, 5);
                    foreach (Button navigationButton in navigationButtons)
                        SetControlRounded(navigationButton, roundedControlsEnabled, 5);
                    foreach (ThemedComboBox comboBox in comboBoxes)
                        SetControlRounded(comboBox, roundedControlsEnabled, 4);
                    SetControlRounded(doneButton, roundedControlsEnabled, 5);
                    SetControlRounded(versionInfoText, roundedControlsEnabled, 5);
                ...
                roundedControlsToggle.CheckedChanged += delegate
                {
                    roundedControlsEnabled = roundedControlsToggle.Checked;
                    ApplyTheme();
                    SaveSettingsFile();
                    applyDialogTheme();
                };
                ...
                {
                    logoStyle = logoStyleBox.SelectedIndex == 1 ? "Professional" : "Current";
                    ApplySelectedLogo();
                    Image oldImage = appLogo.Image;
                    appLogo.Image = appIcon.ToBitmap();
                    if (oldImage != null)
                        oldImage.Dispose();
                    SaveSettingsFile();
        ...
        private void ApplyMainControlCorners()
        {
            foreach (Control control in new Control[]
            {
                statusLabel, shortcutValueLabel, shortcutTwoValueLabel,
                shortcutButton, shortcutTwoButton, settingsButton, topButton, refreshButton, hideButton
            })
                SetControlRounded(control, roundedControlsEnabled, 5);
        }

        private static void SetControlRounded(Control control, bool enabled, int radius)
        {
            Region previous = control.Region;
            if (!enabled || control.Width < 4 || control.Height < 4)
            {
                control.Region = null;
                if (previous != null)
                    previous.Dispose();
