...
namespace PulseMute
{
    ...
    internal sealed partial class MainForm : Form
    {
        ...
        private const string ReleaseVersion = "26.1.0.20-beta";
    ...
    internal sealed class RoundButton : Button
    {
        ...
        protected override void OnPaint(PaintEventArgs pevent)
        {
            ...
            else
            {
                ...
                Rectangle textBounds = VisualStyle == 2
                    ? new Rectangle((int)(Width * 0.47F), (int)(Height * 0.28F), (int)(Width * 0.43F), (int)(Height * 0.44F))
                    : new Rectangle(8, (int)(Height * 0.65F), Math.Max(1, Width - 16), Math.Max(1, (int)(Height * 0.25F)));
                TextRenderer.DrawText(
                    g,
                    Text,
                    Font,
        ...
        private GraphicsPath CreateShapePath(Rectangle bounds)
        {
            ...
            if (VisualStyle == 1)
            {
                RectangleF capsule = new RectangleF(
                    ...
                    bounds.Height);
                path.Dispose();
                return RoundedRectanglePath(capsule, capsule.Width / 2F);
            }
            ...
            if (VisualStyle == 2)
            {
                RectangleF pill = new RectangleF(
                    ...
                    bounds.Height * 0.6F);
                path.Dispose();
                return RoundedRectanglePath(pill, pill.Height / 2F);
            }
            ...
            if (VisualStyle == 4)
            {
                float bubbleRadius = Math.Max(8F, Math.Min(bounds.Width, bounds.Height) * 0.16F);
                float bubbleDiameter = bubbleRadius * 2F;
                float bodyBottom = bounds.Top + bounds.Height * 0.8F;
                path.StartFigure();
                path.AddLine(bounds.Left + bubbleRadius, bounds.Top, bounds.Right - bubbleRadius, bounds.Top);
                path.AddArc(bounds.Right - bubbleDiameter, bounds.Top, bubbleDiameter, bubbleDiameter, 270, 90);
                path.AddLine(bounds.Right, bounds.Top + bubbleRadius, bounds.Right, bodyBottom - bubbleRadius);
                path.AddArc(bounds.Right - bubbleDiameter, bodyBottom - bubbleDiameter, bubbleDiameter, bubbleDiameter, 0, 90);
                path.AddLine(bounds.Right - bubbleRadius, bodyBottom, bounds.Left + bounds.Width * 0.62F, bodyBottom);
                path.AddLine(bounds.Left + bounds.Width * 0.62F, bodyBottom, bounds.Left + bounds.Width * 0.52F, bounds.Bottom);
                path.AddLine(bounds.Left + bounds.Width * 0.52F, bounds.Bottom, bounds.Left + bounds.Width * 0.46F, bodyBottom);
                path.AddLine(bounds.Left + bounds.Width * 0.46F, bodyBottom, bounds.Left + bubbleRadius, bodyBottom);
                path.AddArc(bounds.Left, bodyBottom - bubbleDiameter, bubbleDiameter, bubbleDiameter, 90, 90);
                path.AddLine(bounds.Left, bodyBottom - bubbleRadius, bounds.Left, bounds.Top + bubbleRadius);
                path.AddArc(bounds.Left, bounds.Top, bubbleDiameter, bubbleDiameter, 180, 90);
                path.CloseFigure();
                return path;
            }

            int baseSize = Math.Min(bounds.Width, bounds.Height);
            int radius = ShapeStyle == 1 ? Math.Max(8, bounds.Height / 2) : Math.Max(8, baseSize / 7);
        ...
        private void DrawVisualGlyph(Graphics graphics, int style, bool muted)
        {
            ...
            float centerX = style == 2 ? Width * 0.29F : Width / 2F;
            float centerY = style == 2 ? Height * 0.5F : Height * 0.38F;
            ...
            using (SolidBrush brush = new SolidBrush(ForeColor))
            {
                ...
                {
                    RectangleF capsule = new RectangleF(centerX - 8F * unit, centerY - 20F * unit, 16F * unit, 29F * unit);
                    using (GraphicsPath micPath = RoundedRectanglePath(capsule, 8F * unit))
                        graphics.FillPath(brush, micPath);
                    graphics.DrawArc(pen, centerX - 15F * unit, centerY - 5F * unit, 30F * unit, 24F * unit, 0, 180);
                    graphics.DrawLine(pen, centerX, centerY + 19F * unit, centerX, centerY + 25F * unit);
                }
                else if (style == 4)
                {
                    graphics.FillEllipse(brush, centerX - 3F * unit, centerY + 8F * unit, 6F * unit, 6F * unit);
                    graphics.DrawArc(pen, centerX - 15F * unit, centerY - 4F * unit, 30F * unit, 30F * unit, 205, 130);
                    graphics.DrawArc(pen, centerX - 25F * unit, centerY - 14F * unit, 50F * unit, 50F * unit, 210, 120);
                }
                else
                {
