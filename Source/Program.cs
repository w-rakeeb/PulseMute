...
namespace PulseMute
{
    ...
    internal sealed partial class MainForm : Form
    {
        ...
        private const string ReleaseVersion = "26.1.0.9-stable";
        ...
        private int muteButtonDesign = 5;
        ...
        internal static Icon LoadAppIcon(int style)
        {
            style = NormalizeLogoStyle(style);
            try
            {
                using (Stream stream = Assembly.GetExecutingAssembly().GetManifestResourceStream(LogoIconResourceName(style)))
                {
                    if (stream != null)
                    {
                        using (Icon icon = new Icon(stream, 256, 256))
        ...
        private Icon ApplySelectedLogo()
        {
            Icon nextIcon = LoadAppIcon(logoStyle);
            Icon previousIcon = appIcon;
            appIcon = nextIcon;
            Icon = nextIcon;
            if (tray != null)
                tray.Icon = nextIcon;
            return ReferenceEquals(previousIcon, nextIcon) ? null : previousIcon;
        }

        private static int NormalizeLogoStyle(int style)
        {
            return Math.Max(0, Math.Min(3, style));
        }

        private static string LogoIconResourceName(int style)
        {
            if (style == 1) return "PulseMuteLogoShadow.ico";
            if (style == 2) return "PulseMuteLogoRounded.ico";
            if (style == 3) return "PulseMuteLogoOutline.ico";
            return "PulseMuteProfessional.ico";
        }
        ...
        internal static Bitmap CreateHighQualityLogoBitmap(int style = 0)
        {
            try
            {
                string resourceName = "PulseMuteProfessional.png";
                style = NormalizeLogoStyle(style);
                if (style == 1) resourceName = "PulseMuteLogoShadow.png";
                else if (style == 2) resourceName = "PulseMuteLogoRounded.png";
                else if (style == 3) resourceName = "PulseMuteLogoOutline.png";
        ...
        private static string EditionVersionText()
        {
            ...
            return "PulseMute " + ReleaseVersion;
        }
        ...
        private void ShowMicrophoneUnavailableState()
        {
            statusLabel.Text = "No mic";
            statusLabel.BackColor = Color.FromArgb(103, 65, 25);
            deviceLabel.Text = "No microphone detected";
            ...
            toggleButton.Invalidate();
            tray.Icon = appIcon;
            tray.Text = "PulseMute - no microphone detected";
        }
        ...
        private void HideToTray()
        {
            ...
            tray.ShowBalloonTip(900, "PulseMute", "Listening for " + CurrentShortcutText(1) + " or " + CurrentShortcutText(2) + ".", ToolTipIcon.Info);
        }
        ...
        private void LoadWindowSettings()
        {
            ...
            try
            {
                ...
                foreach (string line in lines)
                {
                    ...
                    else if (line.StartsWith("LogoStyle=", StringComparison.OrdinalIgnoreCase))
                    {
                        int.TryParse(line.Substring("LogoStyle=".Length), out logoStyle);
                    }
                ...
                muteButtonDesign = Math.Max(0, Math.Min(5, muteButtonDesign));
                logoStyle = NormalizeLogoStyle(logoStyle);
        ...
        private void SaveSettingsFile()
        {
            ...
            File.WriteAllText(
                ...
                "LogoStyle=" + logoStyle + Environment.NewLine +
        ...
        internal static List<ArchivedVersionInfo> DiscoverArchivedVersions(string startupPath)
        {
            ...
            string[] channelArchives =
            {
                Path.Combine(startupPath, "Main source", "Version Archive"),
                Path.Combine(startupPath, "Source", "Version Archive")
            };
            foreach (string channelArchive in channelArchives)
            ...
            {
                Path.Combine(projectRoot, "PulseMute Archives"),
                Path.Combine(startupPath, "Legacy Versions")
            };
            foreach (string legacyArchive in legacyArchives)
        ...
        private static string ArchivedVersionDetails(string name)
        {
            ...
            if (name == "Beta 26.1.0.21-beta") return "Restored the preferred mute-control designs and finalized the promoted Beta feature set.";
            if (name == "Beta 26.1.0.22-beta") return "Introduced the red muted-microphone identity across the Beta application.";
    ...
    {
        private static readonly Bitmap MutedControlArtwork = LoadControlArtwork("PulseMuteControlMuted.png");
        private static readonly Bitmap LiveControlArtwork = LoadControlArtwork("PulseMuteControlLive.png");
        ...
        protected override void OnPaint(PaintEventArgs pevent)
        {
            ...
            bool muted = string.Equals(Text, "Unmute", StringComparison.OrdinalIgnoreCase);
            if (VisualStyle == 5)
            {
                DrawStateControlArtwork(g, face, muted);
                if (Focused)
            ...
            {
                DrawVisualGlyph(g, VisualStyle, muted);
        ...
        private static Bitmap LoadControlArtwork(string resourceName)
        {
            try
            {
                using (Stream stream = Assembly.GetExecutingAssembly().GetManifestResourceStream(resourceName))
        ...
        private void DrawStateControlArtwork(Graphics graphics, Rectangle bounds, bool muted)
        {
            Bitmap artwork = muted ? MutedControlArtwork : LiveControlArtwork;
            if (artwork == null)
            {
            ...
            int side = Math.Max(1, Math.Min(bounds.Width, bounds.Height));
            ...
            graphics.DrawImage(artwork, target);
        }
