...
namespace PulseMute
{
    internal static class ControllerProtocolTests
    {
        ...
        private static void Main()
        {
            ...
            TestUnavailableMicrophonePresentation();
            TestInstanceIsolation();
            TestCompactSettingsCardLayout();
            TestMuteButtonVisuals();
            TestSettingsPlacement();
        ...
        private static void TestResponsiveHotkeyLayout()
        {
            ...
            System.Reflection.FieldInfo hotkeyStripAboveCredit = type.GetField("hotkeyStripAboveCredit", flags);
            ...
            try
            {
                ...
                hotkeyStripAboveCredit.SetValue(form, true);
                applyLayout.Invoke(form, null);
                Assert(keyOne.Bottom < credit.Top, "Upper hotkey position was not restored above the creator line.");
                Assert(keyOne.Left == refresh.Left, "Upper Key 1 card did not restore the previous left edge.");
                Assert(keyTwo.Right == hide.Right, "Upper Key 2 card did not restore the previous right edge.");
                Assert(keyOne.Height >= 28 && keyTwo.Height >= 28, "Upper hotkey cards did not restore their full height.");
        ...
        private static void TestInstanceIsolation()
        {
            ...
            string previousReleaseConfig = MainForm.ConfigPathForRelease(stablePath, appData, "Stable", "26.1.0.8-stable");
            string currentReleaseConfig = MainForm.ConfigPathForRelease(stablePath, appData, "Stable", "26.1.0.9-stable");
            ...
            Assert(MainForm.ReleaseSettingsIdentity().IndexOf("Stable-26_1_0_9-stable", StringComparison.OrdinalIgnoreCase) >= 0, "Current Stable release identity is invalid.");
        }

        private static void TestUnavailableMicrophonePresentation()
        {
            MainForm form = new MainForm();
            try
            {
                System.Reflection.MethodInfo showUnavailable = typeof(MainForm).GetMethod(
                    "ShowMicrophoneUnavailableState",
                    System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic);
                Assert(showUnavailable != null, "Missing-microphone presentation helper is absent.");
                showUnavailable.Invoke(form, null);

                System.Reflection.BindingFlags fields = System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic;
                System.Windows.Forms.Label status = (System.Windows.Forms.Label)typeof(MainForm).GetField("statusLabel", fields).GetValue(form);
                System.Windows.Forms.Label device = (System.Windows.Forms.Label)typeof(MainForm).GetField("deviceLabel", fields).GetValue(form);
                System.Windows.Forms.Control toggle = (System.Windows.Forms.Control)typeof(MainForm).GetField("toggleButton", fields).GetValue(form);
                Assert(status.Text == "No mic", "Missing-microphone status badge is unclear.");
                Assert(device.Text == "No microphone detected", "Raw Windows microphone error is still visible.");
                Assert(!toggle.Enabled && toggle.Text == "No mic", "Mute control remains active without a microphone.");
            }
            finally
            {
                form.Dispose();
        ...
        {
            System.Windows.Forms.Panel page = new System.Windows.Forms.Panel();
            page.Size = new System.Drawing.Size(480, 420);
            System.Windows.Forms.Label title = new System.Windows.Forms.Label();
            title.Text = "Remember window placement";
            title.SetBounds(28, 88, 280, 22);
            ...
            state.Text = "Enabled";
            state.SetBounds(28, 110, 180, 18);
            state.Anchor = System.Windows.Forms.AnchorStyles.Top | System.Windows.Forms.AnchorStyles.Left | System.Windows.Forms.AnchorStyles.Right;
            ToggleSwitch toggle = new ToggleSwitch();
            toggle.SetBounds(510, 94, 46, 24);
            toggle.Anchor = System.Windows.Forms.AnchorStyles.Top | System.Windows.Forms.AnchorStyles.Right;
            ...
            Assert(cards.Count == 1 && cards[0].Width == 436, "Settings card did not use the real content width.");
            Assert(title.Width >= 300 && state.Width >= 300, "Settings text collapsed beside its toggle.");
            Assert(toggle.Right <= cards[0].ClientSize.Width - 12, "Settings toggle escaped its card.");
            Assert(System.Windows.Forms.TextRenderer.MeasureText(title.Text, title.Font).Width <= title.Width, "A full Settings title is still clipped.");
            page.Dispose();
        }

        private static void TestMuteButtonVisuals()
        {
            System.Collections.Generic.HashSet<int> signatures = new System.Collections.Generic.HashSet<int>();
            for (int style = 0; style < 6; style++)
            {
                using (RoundButton button = new RoundButton())
                using (System.Drawing.Bitmap bitmap = new System.Drawing.Bitmap(126, 126))
                {
                    button.Size = bitmap.Size;
                    button.Text = "Mute";
                    button.ForeColor = System.Drawing.Color.White;
                    button.VisualStyle = style;
                    button.DrawToBitmap(bitmap, new System.Drawing.Rectangle(System.Drawing.Point.Empty, bitmap.Size));
                    int signature = 17;
                    for (int y = 8; y < bitmap.Height; y += 11)
                        for (int x = 8; x < bitmap.Width; x += 11)
                            signature = unchecked(signature * 31 + bitmap.GetPixel(x, y).ToArgb());
                    Assert(signatures.Add(signature), "Mute control visual " + style + " duplicated another design.");
                }
            }

            using (RoundButton button = new RoundButton())
            using (System.Drawing.Bitmap mutedBitmap = new System.Drawing.Bitmap(126, 126))
            using (System.Drawing.Bitmap liveBitmap = new System.Drawing.Bitmap(126, 126))
            {
                button.Size = mutedBitmap.Size;
                button.ForeColor = System.Drawing.Color.White;
                button.VisualStyle = 5;
                button.Text = "Unmute";
                button.DrawToBitmap(mutedBitmap, new System.Drawing.Rectangle(System.Drawing.Point.Empty, mutedBitmap.Size));
                button.Text = "Mute";
                button.DrawToBitmap(liveBitmap, new System.Drawing.Rectangle(System.Drawing.Point.Empty, liveBitmap.Size));
                System.Drawing.Color mutedColor = mutedBitmap.GetPixel(20, 63);
                System.Drawing.Color liveColor = liveBitmap.GetPixel(20, 63);
                Assert(mutedColor.R > mutedColor.G, "State microphone control is not red while muted.");
                Assert(liveColor.G > liveColor.R, "State microphone control is not green while live.");
            }

            using (System.IO.Stream stream = System.Reflection.Assembly.GetExecutingAssembly().GetManifestResourceStream("PulseMuteControlMuted.png"))
            {
                Assert(stream != null, "Corrected muted-control resource is missing.");
                using (System.Drawing.Bitmap bitmap = new System.Drawing.Bitmap(stream))
                {
                    System.Drawing.Color correctedRed = bitmap.GetPixel(20, 256);
                    Assert(correctedRed.R == 160 && correctedRed.G == 42 && correctedRed.B == 57,
                        "Muted-control background is not the corrected #A02A39 color.");
                }
            }
        }

        private static void TestSettingsPlacement()
        {
            System.Drawing.Rectangle workArea = new System.Drawing.Rectangle(0, 0, 1920, 1080);
            System.Drawing.Size settingsSize = new System.Drawing.Size(660, 500);

            System.Drawing.Rectangle leftOwner = new System.Drawing.Rectangle(100, 120, 300, 380);
            System.Drawing.Point rightPlacement = MainForm.CalculateSettingsLocation(leftOwner, settingsSize, workArea, 14);
            Assert(rightPlacement.X == leftOwner.Right + 14, "Settings did not open to the right when space was available.");
            Assert(rightPlacement.Y == leftOwner.Top, "Right-side Settings placement lost owner alignment.");

            System.Drawing.Rectangle rightOwner = new System.Drawing.Rectangle(1600, 140, 280, 380);
            System.Drawing.Point leftPlacement = MainForm.CalculateSettingsLocation(rightOwner, settingsSize, workArea, 14);
            Assert(leftPlacement.X == rightOwner.Left - 14 - settingsSize.Width, "Settings did not fall back to the left side.");
            Assert(leftPlacement.Y == rightOwner.Top, "Left-side Settings placement lost owner alignment.");

            System.Drawing.Rectangle bottomOwner = new System.Drawing.Rectangle(100, 900, 300, 160);
            System.Drawing.Point clampedPlacement = MainForm.CalculateSettingsLocation(bottomOwner, settingsSize, workArea, 14);
            Assert(clampedPlacement.Y == workArea.Bottom - settingsSize.Height, "Settings escaped the monitor bottom edge.");
            Assert(workArea.Contains(new System.Drawing.Rectangle(clampedPlacement, settingsSize)), "Settings placement escaped the monitor work area.");
        }
        ...
        private static void TestNativeSettingsDesign()
        {
            ...
            string sourceDirectory = System.IO.Path.Combine(appDirectory, "Main source");
            if (!System.IO.Directory.Exists(sourceDirectory))
                sourceDirectory = System.IO.Path.Combine(appDirectory, "Source");
            string sourcePath = System.IO.Path.Combine(sourceDirectory, "SidebarSettings.cs");
            string source = System.IO.File.ReadAllText(sourcePath);
            string programSource = System.IO.File.ReadAllText(System.IO.Path.Combine(sourceDirectory, "Program.cs"));
            ...
            Assert(source.IndexOf("26.1.0.9-stable", StringComparison.Ordinal) >= 0, "Settings version label is stale.");
            Assert(source.IndexOf("PulseMute settings", StringComparison.Ordinal) >= 0, "PulseMute Settings branding is missing.");
            ...
            Assert(source.IndexOf("Microphone tile", StringComparison.Ordinal) >= 0 && source.IndexOf("Wave badge", StringComparison.Ordinal) >= 0, "Restored microphone visuals are missing.");
            Assert(source.IndexOf("Record button", StringComparison.Ordinal) >= 0 && source.IndexOf("Power ring", StringComparison.Ordinal) >= 0, "Restored record and power visuals are missing.");
            Assert(source.IndexOf("Red/green mic", StringComparison.Ordinal) >= 0, "State microphone control is missing from Customization.");
            Assert(programSource.IndexOf("private int muteButtonDesign = 5;", StringComparison.Ordinal) >= 0, "Red/green mic is not the default mute control.");
            Assert(programSource.IndexOf("private int logoStyle;", StringComparison.Ordinal) >= 0, "Supplied primary artwork is not the default app logo.");
            Assert(System.IO.File.ReadAllText(System.IO.Path.Combine(sourceDirectory, "build.ps1")).IndexOf("$icon = Join-Path $root \"PulseMute.ico\"", StringComparison.Ordinal) >= 0,
                "Supplied primary artwork is not the executable icon.");
            Assert(System.IO.File.Exists(System.IO.Path.Combine(sourceDirectory, "PulseMute-primary.svg")), "Supplied primary SVG source is missing.");
            Assert(source.IndexOf("Studio capsule", StringComparison.Ordinal) < 0 && source.IndexOf("Broadcast bubble", StringComparison.Ordinal) < 0, "Rejected mute silhouettes are still selectable.");
            Assert(source.IndexOf("AddSettingsCard(audioPage, cards, 240, 76", StringComparison.Ordinal) >= 0, "Sound-style card scaling fix is missing.");
            Assert(source.IndexOf("AddSettingsCard(audioPage, cards, 328, 90", StringComparison.Ordinal) >= 0, "Feedback-volume card is missing.");
            Assert(source.IndexOf("soundVolumeIcon.Text = \"\\uE767\"", StringComparison.Ordinal) >= 0, "Feedback-volume icon is missing.");
            Assert(source.IndexOf("AddSettingsCard(developerContent", StringComparison.Ordinal) >= 0, "Developer controls are not contained in scrollable cards.");
            Assert(source.IndexOf("Segoe UI Variable Text", StringComparison.Ordinal) >= 0, "Sharper Settings body typography is missing.");
            Assert(source.IndexOf("Segoe UI Variable Display", StringComparison.Ordinal) >= 0, "Sharper Settings display typography is missing.");
            Assert(programSource.IndexOf("MuteButtonDesign=", StringComparison.Ordinal) >= 0, "Mute control design is not persisted.");
            Assert(programSource.IndexOf("LogoStyle=", StringComparison.Ordinal) >= 0, "App logo selection is not persisted.");
            Assert(source.IndexOf("App logo", StringComparison.Ordinal) >= 0, "App logo selector is missing from Customization.");
            Assert(programSource.IndexOf("ConfigPathForRelease", StringComparison.Ordinal) >= 0, "Release-specific Settings isolation is missing.");
            Assert(programSource.IndexOf("ReleaseMutexName", StringComparison.Ordinal) >= 0, "Per-release duplicate-instance protection is missing.");
            Assert(programSource.IndexOf("No microphone detected", StringComparison.Ordinal) >= 0, "Friendly missing-microphone state is absent.");
            Assert(programSource.IndexOf("ShowMicrophoneUnavailableState();", StringComparison.Ordinal) >= 0, "Mute actions do not use the safe unavailable state.");
            Assert(programSource.IndexOf("if (result >= 0)", StringComparison.Ordinal) >= 0,
                "Disconnected selected microphones do not fall back to a Windows default endpoint.");
            ...
            Assert(source.IndexOf("Hotkey strip position", StringComparison.Ordinal) >= 0, "Developer hotkey-position option is missing.");
            Assert(programSource.IndexOf("HotkeyStripAboveCredit=", StringComparison.Ordinal) >= 0, "Hotkey-position preference is not persisted.");
            Assert(programSource.IndexOf("ShowHotkeyStrip=", StringComparison.Ordinal) < 0, "Removed hotkey visibility setting is still active.");
        ...
        private static void TestProfessionalLogoResource()
        {
            ...
            using (System.IO.Stream stream = System.Reflection.Assembly.GetExecutingAssembly().GetManifestResourceStream("PulseMuteProfessional.ico"))
            {
                ...
                using (System.Drawing.Icon icon = new System.Drawing.Icon(stream, 256, 256))
                {
            ...
            using (System.Drawing.Bitmap logo = MainForm.CreateHighQualityLogoBitmap())
            {
                Assert(logo.Width == 512 && logo.Height == 512, "Supplied About logo resource dimensions are invalid.");
                System.Drawing.Color primaryRed = logo.GetPixel(20, 256);
                Assert(primaryRed.R == 160 && primaryRed.G == 42 && primaryRed.B == 57, "Primary logo is not the corrected #A02A39 artwork.");
            }

            string[] iconResources = { "PulseMuteLogoShadow.ico", "PulseMuteLogoRounded.ico", "PulseMuteLogoOutline.ico" };
            foreach (string resourceName in iconResources)
            {
                using (System.IO.Stream stream = System.Reflection.Assembly.GetExecutingAssembly().GetManifestResourceStream(resourceName))
                {
                    Assert(stream != null && stream.Length > 1000, "Selectable icon resource is missing: " + resourceName);
                    using (System.Drawing.Icon icon = new System.Drawing.Icon(stream))
                    {
                        Assert(icon.Width >= 16 && icon.Height >= 16, "Selectable icon dimensions are invalid: " + resourceName);
                        using (System.Drawing.Bitmap bitmap = icon.ToBitmap())
                            Assert(bitmap.Width > 0 && bitmap.Height > 0, "Selectable icon could not render: " + resourceName);
                    }
                }
            }
            ...
            {
                using (System.Drawing.Icon icon = MainForm.LoadAppIcon(style))
                {
                    Assert(icon != null && icon.Width >= 16 && icon.Height >= 16, "Runtime logo icon failed for style " + style + ".");
                    using (System.Drawing.Bitmap bitmap = MainForm.CreateSafeIconBitmap(icon))
                        Assert(bitmap.Width > 0 && bitmap.Height > 0, "Runtime logo icon could not render for style " + style + ".");
                }
            }

            System.Collections.Generic.HashSet<long> logoSignatures = new System.Collections.Generic.HashSet<long>();
            for (int style = 0; style < 4; style++)
            {
                using (System.Drawing.Bitmap logo = MainForm.CreateHighQualityLogoBitmap(style))
                {
                    Assert(logo.Width == 512 && logo.Height == 512, "Selectable logo dimensions are invalid for style " + style + ".");
                    long signature = 17;
                    for (int y = 24; y < logo.Height; y += 47)
                    {
                        for (int x = 24; x < logo.Width; x += 43)
                            signature = unchecked((signature * 31) + logo.GetPixel(x, y).ToArgb());
                    }
                    Assert(logoSignatures.Add(signature), "Selectable logo style " + style + " duplicates another logo.");
                }
            }
        }
